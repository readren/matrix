...
object SerializerDerivation {
	...
	inline def deriveSerializer[A]: Serializer[A] = ${ deriveSerializerImpl[A] }

	private def deriveSerializerImpl[A: Type](using quotes: Quotes): Expr[Serializer[A]] = {
	...
	private def deriveProductSerializer[A: Type, ElemTypes: Type, ElemLabels: Type](using quotes: Quotes): Expr[Serializer[A]] = {
		...
		def block(messageExpr: Expr[A], writerExpr: Expr[Writer])(using quotes: Quotes): Expr[Unit] = {
			import quotes.reflect.*
			...
			def loop[RemainingElemTypes: Type, RemainingElemLabels: Type]: Expr[Unit] = {
				(Type.of[RemainingElemTypes], Type.of[RemainingElemLabels]) match {
					case ('[headType *: tailTypes], '[headLabel *: tailLabels]) =>
						...
						val fieldExpr: Expr[headType] = Select.unique(messageExpr.asTerm, fieldName).asExprOf[headType]

						Expr.summon[Serializer[headType]] match {
							case Some(serializer) => '{
								$serializer.serialize($fieldExpr, $writerExpr)
		...
		'{
			new Serializer[A] {
				def serialize(message: A, writer: Writer): Unit = ${ block('message, 'writer) }
			}
		}
	}
	...
	private def deriveSumSerializer[A: Type, ElemTypes: Type](mirrorExpr: Expr[Mirror.SumOf[A]])(using quotes: Quotes): Expr[Serializer[A]] = {
		...
		def body(messageExpr: Expr[A], writerExpr: Expr[Writer])(using quotes: Quotes): Expr[Unit] = {
			import quotes.reflect.*
			...
			def loop[RemainingElemTypes: Type](index: Int): List[CaseDef] = {
				Type.of[RemainingElemTypes] match {
					case '[headType *: tailTypes] =>
						val serializer = Expr.summon[Serializer[headType]].getOrElse(deriveSerializerImpl[headType])
