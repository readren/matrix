...
import cluster.channel.Deserializer.ValueOrReferenceTest
...
object CommonSerializers {
	...
	//// Set
	given setSerializer[E](using sE: Serializer[E]): Serializer[Set[E]] =
	(set: Set[E], writer: Serializer.Writer) => {
		writer.putIntVlq(set.size)
		set.foreach(e => sE.serialize(e, writer))
	}

	given setDeserializer[E](using dE: Deserializer[E], vorE: ValueOrReferenceTest[E]): Deserializer[Set[E]] =
		(reader: Deserializer.Reader) => {
	...
	//// Map
	given mapSerializer[K, V](using sK: Serializer[K], sV: Serializer[V]): Serializer[Map[K, V]] =
	...
	given mapDeserializer[K, V](using dK: Deserializer[K], dV: Deserializer[V], vorK: ValueOrReferenceTest[K], vorV: ValueOrReferenceTest[V]): Deserializer[Map[K, V]] =
		(reader: Deserializer.Reader) => {
			var count = reader.readIntVlq()
