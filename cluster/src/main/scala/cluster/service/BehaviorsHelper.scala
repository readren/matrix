...
trait BehaviorsHelper { thisCommunicableDelegate: CommunicableDelegate =>
	...
	private[service] def handleHelloCommonLogic(hello: Hello): Boolean = {
		...
		if getAgreedVersion == ProtocolVersion.NOT_SPECIFIED then {
			...
			transmitToPeerOrRestartChannel(Welcome(hello.requestId, participantService.myMembershipStatus, participantService.config.versionsISupport, participantService.myCreationInstant, participantService.getKnownParticipantsAddresses))
			true
		}
	}
	...
	private[service] def handleMessage(message: AreYouInSyncWithMe): Unit = {
		if message.yourMembershipStatusAccordingToMe ne participantService.myMembershipStatus then incitePeerToUpdateHisStateAboutMyStatus()
	...
	private[service] def incitePeerToUpdateHisStateAboutMyStatus(): Unit = {
		transmitToPeerOrRestartChannel(WaitMyMembershipStatusIs(participantService.myMembershipStatus, participantService.getHandshookParticipantsMembershipStatus.toMap))
	}
			...
			delegateByAddress.getOrElse(participantAddress, null) match {
				...
				case null =>
					if participantAddress == participantService.myAddress then {
						if participantMembershipStatusAccordingToPeer ne participantService.myMembershipStatus then incitePeerToUpdateHisStateAboutMyStatus()
					} else {
