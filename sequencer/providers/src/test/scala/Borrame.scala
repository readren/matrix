...
object Borrame {
	...
	@main def borrameRun(): Unit = {
		...
		laDog.subscribeUncancellable()(a => println(a))

		val laDogMutatedToCat: doer.LatchingTask[Cat] = laDog.map(_ => cat)

		laDogMutatedToCat.subscribeUncancellable()(b => println(b))

		val dOne = doer.Task_mine(() => 1)
		val covenant = doer.Covenant[Int]()
		covenant.subscribeUncancellable()(x => println(s"covenant completed with $x"))
		...
		steps.subscribeUncancellable()(r => println(r))
		provider.shutdown()
	}
