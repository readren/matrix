...
class CooperativeThreadDrivenSchedulerDpTest extends DoerProviderTestBase[SchedulingDoerFacade]
	...
	override protected def buildDoerProvider(poolSize: Int): DP = new CooperativeThreadDrivenSchedulerDp(applyMemoryFence = false, threadPoolSize = poolSize, threadFactory = new TestThreadFactory) {
