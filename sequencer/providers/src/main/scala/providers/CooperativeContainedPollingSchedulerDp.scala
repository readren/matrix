...
import providers.CooperativeContainedPollingSchedulerDp.{NOT_ACTIVATED, ScheduleFacade, SchedulingDoerFacade}
...
object CooperativeContainedPollingSchedulerDp extends CooperativeSchedulerDpCompanion {
	final class Impl(
	...
	) extends CooperativeContainedPollingSchedulerDp(applyMemoryFence, threadPoolSize, threadFactory, clock, trackSleepTime) {
...
abstract class CooperativeContainedPollingSchedulerDp(
	applyMemoryFence: Boolean = true,
	threadPoolSize: Int = Runtime.getRuntime.availableProcessors(),
	threadFactory: ThreadFactory = Executors.defaultThreadFactory(),
	clock: MonotonicClock = new NanoTimeBasedMilliClock,
	...
	private class ScheduleImpl(val owner: SchedulingDoerImpl, override val initialDelay: MilliDuration, override val interval: MilliDuration, override val isFixedRate: Boolean) extends MinHeapPriorityQueue.Element, ScheduleFacade {
		 ...
		 * Holds [[CooperativeContainedPollingSchedulerDp.NOT_ACTIVATED]] if the schedule has not been activated. */
		...
		def program(assignedWorkerIndex: Int, scheduledTime: MilliTime): Unit = {
			this.assignedWorkerIndex = assignedWorkerIndex
			this.scheduledTime = scheduledTime
			val queue = workerPriorityQueues(assignedWorkerIndex)
			queue.add(this)
			earliestScheduledTimes(assignedWorkerIndex) = queue.peek.scheduledTime
		}
	...
	private class SchedulingDoerImpl(aTag: Tag) extends DoerImpl(aTag), SchedulingDoerFacade { thisDoer =>
		...
		def getActivationSerialAtLastCancelAll: Long = activationSerialAtLastCancelAll
		...
		override def scheduleSequentially(schedule: Schedule, routine: Schedule => Unit): Unit = {
			...
			if cw ne null then scheduleSequentiallyInternal(cw.index, schedule, routine, activationSerial)
			else thisDoer.executeSequentially { () =>
				scheduleSequentiallyInternal(currentWorker.index, schedule, routine, activationSerial)
			}
		}
		...
		private def scheduleSequentiallyInternal(currentWorkerIndex: Int, schedule: Schedule, routine: Schedule => Unit, activationSerial: Long): Unit = {
			...
			if !schedule.isCanceled && activationSerial > activationSerialAtLastCancelAll then {
							...
							thisDoer.executeSequentially { () =>
								if !schedule.isCanceled && schedule.activationSerial.get > activationSerialAtLastCancelAll then {
									...
									if schedule.interval > 0 && !schedule.isCanceled && schedule.activationSerial.get > activationSerialAtLastCancelAll then {
										...
										schedule.program(currentWorker.index, base + schedule.interval)
									}
								}
							}
						}
				...
				schedule.program(currentWorkerIndex, activationTime + schedule.initialDelay)
			}
		}

		override def cancel(schedule: Schedule): Unit = {
			schedule.isCanceled = true
			val awi = schedule.assignedWorkerIndex
			if awi >= 0 && schedule.heapIndex >= 0 then {
				val w = currentWorker
				if (w != null) && w.index == awi then {
					// We're on the same worker thread that owns this schedule's queue — safe to mutate.
					val queue = workerPriorityQueues(awi)
					queue.remove(schedule)
					val next = queue.peek
					earliestScheduledTimes(awi) = if next eq null then clock.MaxValue else next.scheduledTime
				}
			}
		}
	...
	private def pollExpiredSchedules(workerIndex: Int, workerQueue: MinHeapPriorityQueue[ScheduleImpl], currentTime: MilliTime): DoerImpl | Null = {
		...
		while true do {
			...
			if !earliestSchedule.isCanceled && earliestSchedule.activationSerial.get > earliestSchedule.owner.getActivationSerialAtLastCancelAll then {
