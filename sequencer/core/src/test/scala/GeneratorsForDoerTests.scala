...
object GeneratorsForDoerTests {
	...
	def genSchedule[D <: SchedulingDoer](doer: D, maxDuration: Int = 5): Gen[doer.Schedule] = {
		for {
...
class GeneratorsForDoerTests[D <: Doer](val doer: D, doerProvider: DoerProvider[Doer], recursionLevel: Int = 0) {
	...
	def genSuccessfulFluxFrom[A: ClassTag](elements: Seq[A], syncExecutionOnly: Boolean = false): Gen[DoerFluxPart#Flux[A]] = {
		...
		val emitterGen: Gen[Flux[A]] = Gen.const {
			new DefaultFlux[A] {
				override def subscribeSync(downChainObserver: FluxObserver[A]): doer.Subscription = {
		...
		val monosSeqGen: Gen[Flux[A]] = {
			val monos = IArray.from(elements.map(a => doer.Task_ready(a)))
	...
	def genFailingFluxFrom[A: ClassTag](elementsBeforeFailure: Seq[A], ex: Throwable, syncExecutionOnly: Boolean = false): Gen[DoerFluxPart#Flux[A]] = {
		...
		val monosGen: Gen[Flux[A]] = Gen.const {
			val monosList = elementsBeforeFailure.map(a => doer.Task_ready(a)) :+ doer.Task_fail(ex)
	...
	def genFluxFrom[A: ClassTag](elements: Seq[A], failure: Option[Throwable] = None, syncExecutionOnly: Boolean = false): Gen[DoerFluxPart#Flux[A]] = {
		failure match {
	...
	def genFlux[A: ClassTag](syncExecutionOnly: Boolean = false)(using genA: Arbitrary[A]): Gen[DoerFluxPart#Flux[A]] = {
		for {
	...
	given fluxArbitrary: [A: ClassTag] =>Arbitrary[A] => Arbitrary[DoerFluxPart#Flux[A]] = Arbitrary(genFlux())
