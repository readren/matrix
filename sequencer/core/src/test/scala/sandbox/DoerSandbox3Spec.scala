...
import sandbox.DoerSandbox3.{ANOTHER_AFTER, ExecutionSerial, ResultOrigin}
...
class DoerSandbox3Spec extends ScalaCheckEffectSuite {
	...
	private val sandbox = new DoerSandbox3 {
		...
		override def currentlyRunningDoer: Maybe[DoerSandbox3] = {
	...
	test("Task and Capturer factories - asynchronous cancellation verification") {
		runInSequence {
			// Use a custom DoerSandbox3 instance that executes sequentially with queuing (asynchronous behavior)
			class AsyncSandbox extends DoerSandbox3 {
				override type Tag = String
