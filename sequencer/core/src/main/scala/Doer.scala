...
trait Doer { thisDoer =>
	...
	@threadUnsafe lazy val ownSingleThreadExecutionContext = new ExecutionContext {
	...
	trait Duty[+A] { thisDuty =>
		 ...
		 */
		final inline def repeatedUntilSome[B](condition: (Int, A) => Maybe[B], maxRecursionDepthPerExecutor: Int = 9): Duty[B] =
		 ...
		 */
		final inline def repeatedUntilDefined[B](pf: PartialFunction[(Int, A), B], maxRecursionDepthPerExecutor: Int = 9): Duty[B] =
		 ...
		 */
		final inline def repeatedWhileEmpty[S >: A, B](s0: S, condition: (Int, S) => Maybe[B], maxRecursionDepthPerExecutor: Int = 9): Duty[B] =
		 ...
		 * @tparam B the type of the result of this duty.
		 */
		final inline def repeatedWhileUndefined[S >: A, B](s0: S, pf: PartialFunction[(Int, S), B], maxRecursionDepthPerExecutor: Int = 9): Duty[B] =
	...
	trait Task[+A] extends Duty[Try[A]] { thisTask =>
		 ...
		 * */
		inline final def reiteratedHardyUntilSome[B](condition: (Int, Try[A]) => Maybe[Try[B]], maxRecursionDepthPerExecutor: Int = 9): Task[B] =
		 ...
		 * @return a new [[Task]] that, when executed, repeatedly executes this task and applies the `condition` to the task's result until the function's result is [[Maybe.some]]. The result of this task is the contents of said [[Maybe]] unless any execution of the `taskA` or `condition` terminates abruptly in which case this task result is the cause.
		 * */
		inline final def reiteratedUntilSome[B](condition: (Int, A) => Maybe[Try[B]], maxRecursionDepthPerExecutor: Int = 9): Task[B] =
		...
		inline final def reiteratedHardyUntilDefined[B](pf: PartialFunction[(Int, Try[A]), Try[B]], maxRecursionDepthPerExecutor: Int = 9): Task[B] =
		 ...
		 */
		inline final def reiteratedWhileEmpty[S >: A, B](ts0: Try[S], condition: (Int, Try[S]) => Maybe[B], maxRecursionDepthPerExecutor: Int = 9): Task[B] =
		 ...
		 * @tparam B the type of the result of this task.
		 */
		inline final def reiteratedWhileUndefined[S >: A, B](ts0: Try[S], pf: PartialFunction[(Int, Try[S]), B], maxRecursionDepthPerExecutor: Int = 9): Task[B] = {
