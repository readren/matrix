 ...
 * - Advance ordering invariant: The `primaryStateUpdater` passed to [[advance]] runs after all consumers that subscribed to the previous [[Captor]] in the queue (via earlier [[advance]] or [[causalAnchor]]) have completed their synchronous part.
...
class CausalFence[A, D <: Doer](val doer: D)(initialState: A) {
	...
	private var lastEnqueuedCovenant: doer.Captor[A] = lastCommittedCovenant
		...
		if lec eq lcc then {
			...
			lec.subscribeSyncCallbacks(
				a =>
					...
					thisStepCovenant.captureSync(a, completionObserver),
				e =>
					lastCommittedCovenant = thisStepCovenant
					thisStepCovenant.trapSync(e, completionObserver)
			)
			thisStepCovenant
		}
	}
	...
	private def step[B <: A](primaryStateUpdater: (A, RollbackAccessor[B]) => Maybe[doer.Mono[A | B]], isSpeculative: Boolean, isGuarded: Boolean): doer.Capturer[A | B] = {
		doer.checkWithin()
			...
			override def onSuccess(previousState: A): Unit = {
				...
				maybeCaughtError.fold {
					maybeNextStateProviderMono.fold {
						lastCommittedCovenant = thisStepCovenant
						thisStepCovenant.captureSync(previousState)
					} { nextStateProviderTask =>
						nextStateProviderTask.triggerSync(new doer.MonoObserver[A | B] {
							override def onSuccess(newState: A | B): Unit = {
								lastCommittedCovenant = thisStepCovenant
								thisStepCovenant.captureSync(newState)
							}

							override def onError(failure: Throwable): Unit = {
								lastCommittedCovenant = thisStepCovenant
								thisStepCovenant.trapSync(failure)
							}
						})
					}
				} { caughtError =>
					lastCommittedCovenant = thisStepCovenant
					thisStepCovenant.trapSync(caughtError)
				}
			}

			override def onError(e: Throwable): Unit = {
				lastCommittedCovenant = thisStepCovenant
				thisStepCovenant.trapSync(e)
			}
		})
		thisStepCovenant
	}
			...
			override def onSuccess(previousState: A): Unit = {
				...
				maybeCaughtError.fold {
					maybeNextState.fold {
						...
						lastCommittedCovenant = thisStepCovenant
						thisStepCovenant.captureSync(nextState)
					}
				} { caughtError =>
					lastCommittedCovenant = thisStepCovenant
					thisStepCovenant.trapSync(caughtError)
