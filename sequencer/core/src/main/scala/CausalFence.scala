...
class CausalFence[A, D <: Doer](val doer: D)(initialState: A) {
	...
	trait RollbackAccessor[B <: A] {
		 ...
		 */
		def rollback(isWithinDoSerEx: Boolean = doer.isInSequence, onCompleted: (A | B, RollbackApplication) => Unit = (_, _) => ()): doer.LatchingDuty[A | B]
	}
	 ...
	 */
	def committed: doer.LatchingDuty[A] = {
		doer.checkWithin()
	...
	def causalAnchor(stateConsumer: (A, CausalAnchorArrival) => Unit = (_, _) => ()): doer.LatchingDuty[A] = {
		doer.checkWithin()
	...
	inline def advance[B <: A](inline primaryStateUpdater: A => doer.Duty[A | B]): doer.LatchingDuty[A | B] =
	...
	inline def advanceIf[B <: A](inline primaryStateUpdater: A => Maybe[doer.Duty[B]]): doer.LatchingDuty[A | B] = {
	 ...
	 * So, avoid memorizing [[LatchingDuty]] instances returned by [[causalAnchor]] or [[advance]]-like methods; always subscribe to the instance returned by [[causalAnchor]] to ensure causal ordering of the consumers executions.
	 * Obsolete are those instances returned by methods of this [[CausalFence]] before the last call to an [[advance]]-like method. */
	inline def advanceSpeculatively[B <: A](inline primaryStateUpdater: (A, RollbackAccessor[B]) => doer.Duty[A | B]): doer.LatchingDuty[A | B] =
	 ...
	 * @return a [[LatchingDuty]] that yields the updated state
	 * */
	inline def advanceSpeculativelyIf[B <: A](primaryStateUpdater: (A, RollbackAccessor[B]) => Maybe[doer.Duty[A | B]]): doer.LatchingDuty[A | B] =
	...
	private def step[B <: A](primaryStateUpdater: (A, RollbackAccessor[B]) => Maybe[doer.Duty[A | B]], isSpeculative: Boolean): doer.LatchingDuty[A | B] = {
		doer.checkWithin()
	 ...
	 */
	inline def jump[B <: A](inline primaryStateUpdater: A => A | B): doer.LatchingDuty[A | B] =
	 ...
	 * @return a [[LatchingDuty]] that is always fulfilled with the committed state
	 */
	def jumpIf[B <: A](primaryStateUpdater: A => Maybe[A | B]): doer.LatchingDuty[A | B] = {
		doer.checkWithin()
		val previousStepCovenant = lastEnqueuedCovenant
		val thisStepCovenant = doer.Covenant[A | B]()
		lastEnqueuedCovenant = thisStepCovenant
