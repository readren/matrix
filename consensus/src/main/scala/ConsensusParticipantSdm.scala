...
trait ConsensusParticipantSdm { thisModule =>
	...
	class ConsensusParticipant(cluster: ClusterParticipant, storage: Storage, machine: StateMachine, indexOfTheIncludingConfigChange: RecordIndex, participantsInTheIncludingConfigChange: ListSet[ParticipantId], initialListeners: Iterable[NotificationListener]) { thisConsensusParticipant =>
		...
		private abstract class StatefulRole(val primaryStateFence: CausalFence[PrimaryState, sequencer.type]) extends Role { thisStatefulRole =>
			...
			private def handleAppendOutcome(primaryState1: PrimaryState, inquirerId: ParticipantId, inquirerTerm: Term, maybeSnapshot: Maybe[SnapshotData[ParticipantId]], prevRecordIndex: RecordIndex, prevRecordTerm: Term, batch: IArray[Record], leaderCommit: RecordIndex, termAtLeaderCommit: Term, fusionReport: Int)(using Trace.Context): sequencer.Capturer[AppendResult] = {
				...
				primaryState1 match {
					case Inaccessible =>
						illegalStateQuiesce()
						sequencer.Keeper(AppendResult(PRE_INIT, Long.MaxValue, this.ordinal))
					...
					case accessible1: Accessible =>
						...
						if (fusionReport & FR_RECORD_FUSED) != 0 then {
							...
							sequencer.Keeper(AppendResult(accessible1.currentTerm, 0L, currentRole.ordinal))
						}
						// If the snapshot was not updated nor a record was fused, then:
							...
							// Else, surely either the inquirer's term is stale, the terms at prevRecordIndex do not match, the batch fully predates the local latest snapshot, or the cluster is in an illegal state with two leaders. So, respond with a rejection asking for earlier records, pointing to the first record that is missig or we known the term does not match.
								...
								sequencer.Keeper(AppendResult(accessible1.currentTerm, successOrIndexForNextAttempt, currentRole.ordinal))
							}
						}
				}
			}
		...
		private final class Quiesced(val motive: Try[String]) extends Role {
			...
			override def onAppendRecords(inquirerId: ParticipantId, inquirerTerm: Term, prevRecordIndex: RecordIndex, prevRecordTerm: Term, batch: IArray[Record], leaderCommit: RecordIndex, termAtLeaderCommit: Term): sequencer.Capturer[AppendResult] = {
				checkWithin()
				sequencer.Keeper(AppendResult(PRE_INIT, Long.MaxValue, ordinal))
			}
			...
			override def onAppendRecords(inquirerId: ParticipantId, inquirerTerm: Term, prevRecordIndex: RecordIndex, prevRecordTerm: Term, batch: IArray[Record], leaderCommit: RecordIndex, termAtLeaderCommit: Term): sequencer.Capturer[AppendResult] = {
				Trace.init(() => s"$boundParticipantId: Retiring.onAppendRecords") {
					...
					findLastIncludingConfigChangeIn(prevRecordIndex + 1, batch).fold(
						...
						sequencer.Keeper(AppendResult(finalTerm, excludingConfigIndex + 1, ordinal))
					) { findResult =>
				...
				Trace.step("driveLoop") {
					...
					val inquire =
						if indexOfNextRecordToSend > configChangeIndex then {
							...
							id.installSnapshot(currentTerm0, maybeSnapshot.get, potentiallyUnappendedPlainRecords, configChangeIndex, configChangeTerm)
						} else {
					...
					inquire.triggerSync(new sequencer.MonoObserver[AppendResult] {
						...
						override def onSuccess(appendResult: AppendResult): Unit = {
							ifDriverIsStillActiveDo { currentTerm1 =>
								...
								// If the appending was rejected due to an unreversible reason, terminate this driver, scribe a message and become quiesced if eligible.
