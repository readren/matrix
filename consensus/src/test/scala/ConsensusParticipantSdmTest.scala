...
class ConsensusParticipantSdmTest extends ScalaCheckEffectSuite {
		...
		object clock extends MonotonicClock { thisClock =>
			...
			override def suspend(lockObject: Object, duration: MilliDuration): Unit = {
				...
				if suspendCallSerial.get == callSerial then {
					...
					if netSequencer != null then netSequencer.run {
			...
			def loop(previousResponses: List[ConfigChangeResponse], alreadyTriedNodes: List[Id]): netSequencer.LatchingDuty[List[ConfigChangeResponse]] = {
				...
				if remainingTargetNodes.isEmpty then netSequencer.LatchingDuty_ready(previousResponses)
					...
					maybeNextNodeId.fold {
						...
						for {
							response <- netSequencer.Covenant_triggerAndWire(inquire)
		...
		createAndInitializeNodes(net, weakReferencesHolder) { node =>
			...
			new node.DefaultNotificationListener() {
				...
				override def onCommitIndexChanged(previous: RecordIndex, current: RecordIndex, as: RoleOrdinal, at: Term): Unit = {
					...
					val from = if previous >= node.storage.memory.logBufferOffset then previous.toInt else node.storage.memory.logBufferOffset.toInt
					if from < thisNodeCommittedRecordsMemory.size then promise.tryFailure(new AssertionError(s"Node ${node.myId} commited the same record index more than once: previous=$previous, current=$current"))
					else {
						val holeLength = from - thisNodeCommittedRecordsMemory.size
						if holeLength > 0 then thisNodeCommittedRecordsMemory.addAll(Iterable.fill(holeLength)(None))
						val newCommittedRecords = node.storage.memory.getRecordsBetween(from + 1, current + 1)
						...
						if as == LEADER then {
							...
							for nodeIndex <- 0 until clusterSize do {
								...
								if otherNode ne node then {
									...
									for (otherNodeCommitedRecordEntry, recordIndex) <- otherNodeCommittedRecordsMemory.zipWithIndex do {
										otherNodeCommitedRecordEntry match {
											case None => ()
											case otherNodeCommitedRecord: Record =>
												if thisNodeCommittedRecordsMemory.size < recordIndex then promise.tryFailure(new AssertionError(s"Node ${otherNode.myId} has more commited records than the current leader ${node.myId}, which breaks the \"Leader completeness\" invariant. The commited records of each are: ${otherNode.myId} -> $otherNodeCommittedRecordsMemory; ${node.myId} -> $thisNodeCommittedRecordsMemory"))
												else {
													thisNodeCommittedRecordsMemory(recordIndex) match {
														case None => ()
														case leaderCommittedRecord: Record =>
															if leaderCommittedRecord != otherNodeCommitedRecord then promise.tryFailure(new AssertionError(s"Node ${otherNode.myId} has a commited records at index $recordIndex that differs from the record of the current leader ${node.myId}, which breaks the \"Leader completeness\" invariant. The commited records of each are: ${otherNode.myId} -> $otherNodeCommittedRecordsMemory; ${node.myId} -> $thisNodeCommittedRecordsMemory"))
													}
												}
										}
									}
	...
	test("All invariants special case") {
		...
		val (clusterSize, startWithHighestPriorityParticipant, netRandomnessSeed) = (4, false, -2871391883553136003L)
		val net = new Net(clusterSize, randomnessSeed = netRandomnessSeed, requestFailurePercentage = 10, responseFailurePercentage = 10, stimulusSettlingTime = 10)
		scribe.info(s"\n----------------\nBegin: clusterSize=$clusterSize, initialConfig=${net.initialConfigMask.mkString("[", ", ", "]")}, startWithHighestPriorityParticipant=$startWithHighestPriorityParticipant, netRandomnessSeed=$netRandomnessSeed")
		testAllInvariants(net, startWithHighestPriorityParticipant, numberOfCommandsToSend, clusterSize * 10, clusterSize * 10, clusterSize * 10)
	}
