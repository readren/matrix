...
final class Maybe[+A](val value: AnyRef | Null) extends AnyVal {
	...
	inline def map[B](inline f: A => B): Maybe[B] = if isEmpty then Maybe.empty else Maybe.some(f(get))

	inline def flatMap[B](inline f: A => Maybe[B]): Maybe[B] = if isEmpty then Maybe.empty else f(value.asInstanceOf[A])

	inline def fold[B](inline ifEmpty: => B)(inline f: A => B): B = if isEmpty then ifEmpty else f(value.asInstanceOf[A])

	inline def orElse[B >: A](inline maybeB: => Maybe[B]): Maybe[B] = if isEmpty then maybeB else this
...
object Maybe {
	...
	inline def apply[A](a: A | Null): Maybe[A] = new Maybe(a.asInstanceOf[AnyRef | Null])
	...
	def liftPartialFunction[A, B](pf: PartialFunction[A, B]): A => Maybe[B] = (a: A) => if pf.isDefinedAt(a) then Maybe.some(pf.apply(a)) else empty
