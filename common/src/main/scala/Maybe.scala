...
object Maybe {
	...
	extension [A](self: Maybe[A]) {
		...
		inline def map[B](inline f: A => B): Maybe[B] = ${ MaybeMacros.mapImpl[A, B]('self, 'f) }

		inline def flatMap[B](inline f: A => Maybe[B]): Maybe[B] = ${ MaybeMacros.flatMapImpl[A, B]('self, 'f) }
