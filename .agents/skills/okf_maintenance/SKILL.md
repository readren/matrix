...
- **No Bug Diaries**: Never document individual bugs, transient code fixes, solved race conditions, or development progress.
- **Invariant Focus**: Write only about the current stable architecture, constraints, API protocols, and guidelines.
- **Log Scope**: `okf/log.md` must only log updates to the documentation itself (e.g., "Added component documentation for Sequencer"), never the history of code fixes or git commits.