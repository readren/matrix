...
trait Behavior[-M] {
	def handleMessage(message: M): HandleMsgResult[M] = Continue
...
object Behavior {
	...
	final class Union[A, B](bA: Behavior[A], bB: Behavior[B])(using ttA: TypeTest[A | B, A], ttB: TypeTest[A | B, B]) extends Behavior[A | B] { thisUnionBehavior =>
		override def handleMessage(message: A | B): HandleMsgResult[A | B] = {
			message match {
				case ttA(a) =>
		...
		override def handleSignal(signal: Signal): Unit =
