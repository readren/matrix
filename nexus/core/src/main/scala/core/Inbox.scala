...
/** Facade of 
 * Implementation that are intended for a [[ActantCore]] should, whenever this inbox starts to have a pending message, clear the "owner is ready to process" state and immediately after that should also call the [[ActantCore.onInboxBecomesNonempty()]] method passing the first pending message.
...
trait Inbox[+M] {
	 ...
	 * The implementation may assume that this method is called withing the [[Doer]] of the owning [[ActantCore]] only. */
	def withdraw(): Maybe[M]


	/** Checks if there are no pending messages.
	 * Should be called withing the [[Doer]] of the owning [[ActantCore]] only. */
	def maybeNonEmpty: Boolean


	/** 
	 * Exposes the pending messages for diagnostic.
	 * Should be called within the [[Doer]] of the owning [[ActantCore]] only.
