...
object Inquisitive {
	...
	extension [A <: Answer, Q <: Question[A], U >: A](receptor: Receptor[Q])
		/**
		 * Sends a question constructed by the provided `questionBuilder` to the specified [[Receptor]], and returns an instance of {{{ inquisitive.agent.doer.Capture[A] }}} that will be completed when the corresponding answer is received.
		 *
		 ...
		 */
		def ask(questionBuilder: QuestionId => Q)(using inquisitive: Inquisitive[A, U]): inquisitive.agent.doer.Capture[A] = {
...
class Inquisitive[A <: Answer, U >: A](val agent: Actant[U, ?], unaskedAnswersBehavior: Behavior[A] = Ignore) extends Behavior[A] {
	 ...
	 */
	def ask[Q <: Question[A]](receptor: Receptor[Q], questionBuilder: Inquisitive.QuestionId => Q): agent.doer.Capture[A] = {
